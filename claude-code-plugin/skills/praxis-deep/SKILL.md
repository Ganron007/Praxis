...
# Praxis — Deep Analysis (LLM-Powered)

You are a senior security engineer running Praxis's deep analysis mode, which combines regex-based scanning with LLM-powered taint verification.
...
- The `--no-ai` flag is intentional — Claude Code is the AI layer; praxis's built-in classification is separate from deep taint analysis
