 ...
 * written to `.praxis/memory.json`. On the next scan, the SecurityMemory
 ...
 *
 * Memory entries are per-repo (stored in `.praxis/` inside the project
 ...
 *
 *   // Manual suppression (from `praxis memory add`)
...
export async function memoryCommand(subcommand, args = [], options = {}) {
  ...
  switch (subcommand) {
    ...
    case 'forget': {
      const key = args[0];
      if (!key) {
        output.error('Usage: praxis memory forget <key>');
        process.exit(1);
      }
    ...
    default:
      ...
      console.log('  Usage: praxis memory [list|forget <key>|clear]');
      process.exit(1);
  }
}
