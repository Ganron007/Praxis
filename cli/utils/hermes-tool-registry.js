/**
 * Hermes Tool Registry — Praxis × Hermes Agent
 * =================================================
 *
 * Declares Praxis's five security tools in the Hermes Agent tool-registry
 * format. Import this module in your Hermes agent bootstrap to register
 * Praxis as a first-class citizen in the tool registry.
 *
 * USAGE:
...
export const HERMES_TOOLS = [
  {
    name: 'praxis_audit',
    description:
      'Run a Praxis security audit on a local codebase directory. ' +
  ...
  {
    name: 'praxis_get_findings',
    description:
      'Retrieve findings from the last saved Praxis scan report for a project. ' +
  ...
  {
    name: 'praxis_memory_list',
    description:
      'List all entries in the Praxis security memory for a project. ' +
...
/**
 * Register all Praxis tools with a Hermes tool registry instance.
 *
...
export async function registerWithHermes(toolRegistry, options = {}) {
  if (!options.skipVerification) { // praxis-ignore — this is the integrity-check implementation, not a bypass
    const mismatches = verifyIntegrity();
    if (mismatches.length > 0) {
      const msg = mismatches.map(m => `  ${m.tool}: expected ${m.expected}, got ${m.actual}`).join('\n');
      throw new Error(`Praxis tool registry integrity check failed:\n${msg}\n\nThis may indicate a supply-chain attack. Run praxis --version to verify your installation.`);
    }
  }
  ...
  for (const tool of HERMES_TOOLS) {
    ...
    if (!options.quiet) {
      console.log(`  [praxis] Registered tool: ${tool.name}`);
    }
  }
}
