 ...
 *
 * PLAYBOOK FILE: .praxis/playbook.md
 *
 * USAGE:
...
export class ScanPlaybook {
  ...
  show() {
    if (!fs.existsSync(this.playbookPath)) {
      console.log('\n  No playbook yet. Run `praxis audit` at least twice to generate one.\n');
      return;
    }
    const content = fs.readFileSync(this.playbookPath, 'utf-8');
    console.log('\n' + content);
  ...
  addNote(text) {
    if (!fs.existsSync(this.playbookPath)) {
      console.error('  No playbook yet — run praxis audit first.');
      return;
    }
  ...
  _buildPlaybook(recon, history, findings, suppressedRules, customNotes) {
    ...
    return `# Praxis Playbook — ${repoName}
...
The following rules are currently suppressed in \`.praxis/memory.json\`:
...
${customNotes || '## Custom Notes\n\n_Add project-specific context here with: `praxis playbook add-note "..."`_\n'}
`;
  }
}

// =============================================================================
// CLI COMMAND  (praxis playbook)
// =============================================================================
...
export async function playbookCommand(subcommand, args = [], options = {}) {
  ...
  switch (subcommand) {
    ...
    case 'add-note': {
      ...
      if (!text) {
        console.error('  Usage: praxis playbook add-note "your note here"');
        process.exit(1);
      }
    ...
    default:
      ...
      console.log('  Usage: praxis playbook [show|add-note "..."]');
      process.exit(1);
  }
}
