 ...
 *
 * Used by `praxis remediate --all` to auto-fix agent-detected issues
 * beyond just secrets.
 */
...
export const AUTOFIX_RULES = [
  ...
  },
  {
    ...
    match: /dangerouslySetInnerHTML\s*=\s*\{\s*\{\s*__html\s*:\s*([^}]+)\}\s*\}/g, // praxis-ignore: autofix pattern
    replace: (line) => {
      return line.replace(
        /dangerouslySetInnerHTML\s*=\s*\{\s*\{\s*__html\s*:\s*([^}]+)\}\s*\}/,
        (_, value) => `dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(${value.trim()}) }}` // praxis-ignore: replacement template
      );
    },
