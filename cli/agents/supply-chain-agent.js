...
export class SupplyChainAudit extends BaseAgent {
  ...
  async analyze(context) {
    ...
    //    Two layers: (a) the hardcoded COMPROMISED_PACKAGES list for offline
    //    use, and (b) the merged ThreatIntel feed (OSV + GHSA + paid sources)
    //    for live coverage. Matches from either layer are deduped by
    //    (file, package, version).
    const iocSources = [];
    const iocSeen = new Set();
    const addIoc = (entry) => {
      const key = `${entry.file}::${entry.name}@${entry.version}`;
      if (iocSeen.has(key)) return;
      iocSeen.add(key);
      iocSources.push(entry);
    };

    const collectAdvisories = (name, version, ecosystem, file) => {
      const advisories = [];
      try {
        const osv = ThreatIntel.lookupOsv(name, version, ecosystem);
        for (const a of osv) {
          if (a.isAffected === false) continue;
          advisories.push({ source: 'osv', ...a });
        }
        const ghsa = ThreatIntel.lookupGhsa(name, ecosystem);
        for (const a of ghsa) advisories.push({ source: 'ghsa', ...a });
      } catch { /* threat-intel optional; ignore */ }
      return advisories;
    };
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        for (const [name, version] of Object.entries(allDeps)) {
          ...
          // Hardcoded IOC list (offline fallback)
          const ioc = COMPROMISED_PACKAGES.find(c => c.name === name);
          if (ioc && ioc.badVersions.includes(bare)) {
            addIoc({ file: pkgPath, name, version: bare, note: ioc.note, source: 'seed' });
          }

          // Live threat-intel lookup
          for (const adv of collectAdvisories(name, bare, 'npm', pkgPath)) {
            addIoc({
              file: pkgPath, name, version: bare,
              note: adv.summary || adv.note || `${adv.source.toUpperCase()} advisory`,
              source: adv.source,
              cves: adv.cves || (adv.cveId ? [adv.cveId] : []),
    ...
    if (fs.existsSync(path.join(rootPath, 'requirements.txt'))) {
      const reqFile = path.join(rootPath, 'requirements.txt');
      const lines = (this.readFile(reqFile) || '').split('\n');
      for (const line of lines) {
        ...
        const ioc = COMPROMISED_PACKAGES.find(c => c.name === lname);
        if (ioc && ioc.badVersions.includes(version)) {
          addIoc({ file: reqFile, name: lname, version, note: ioc.note, source: 'seed' });
        }

        for (const adv of collectAdvisories(lname, version, 'PyPI', reqFile)) {
          addIoc({
            file: reqFile, name: lname, version,
            note: adv.summary || adv.note || `${adv.source.toUpperCase()} advisory`,
            source: adv.source,
            cves: adv.cves || (adv.cveId ? [adv.cveId] : []),
            advisorySeverity: adv.severity,
    ...
    for (const ioc of iocSources) {
      // Tag with KEV / EPSS for prioritization
      const tags = enrichWithCveSignals(ioc.cves || []);
      const severity = tags.kev ? 'critical' : (ioc.advisorySeverity || 'critical');
      const sourceLabel = ioc.source === 'seed' ? 'bundled IOC list' : ioc.source.toUpperCase();
      const epssNote = tags.maxEpss != null
        ? ` EPSS=${(tags.maxEpss * 100).toFixed(1)}% (${tags.maxEpssCve}).`
        : '';
      const kevNote = tags.kev ? ' Listed in CISA KEV (actively exploited).' : '';
      ...
      findings.push(createFinding({
        ...
        title: `Known-Compromised Package: ${ioc.name}@${ioc.version}`,
        description: `${ioc.name}@${ioc.version} flagged via ${sourceLabel}. ${ioc.note}${kevNote}${epssNote}`,
        matched: `${ioc.name}@${ioc.version}`,
...
/**
 * Look up KEV / EPSS for a list of CVE IDs and return aggregated tags.
 * Used to elevate severity and decorate descriptions on findings that
 * carry a CVE.
 */
function enrichWithCveSignals(cves) {
  const result = { kev: false, kevCves: [], maxEpss: null, maxEpssCve: null };
  for (const cve of cves || []) {
    try {
      if (ThreatIntel.isInKev(cve)) {
        result.kev = true;
        result.kevCves.push(cve);
      }
      const epss = ThreatIntel.getEpss(cve);
      if (epss && (result.maxEpss == null || epss.score > result.maxEpss)) {
        result.maxEpss = epss.score;
        result.maxEpssCve = cve;
      }
    } catch { /* threat-intel not loaded; ignore */ }
  }
  return result;
}
