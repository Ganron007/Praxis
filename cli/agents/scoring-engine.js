...
import { mapFindingToStandards, getStandardsSummary } from '../utils/standards/index.js';
...
export class ScoringEngine {
  ...
  compute(findings = [], depVulns = []) {
    ...
    for (const finding of findings) {
      ...
      finding.standards = mapFindingToStandards(finding);
    }
    ...
    // ── AI security standards summary (OWASP LLM, ATLAS, NIST AI 600-1, ...)
    let standardsSummary;
    try {
      standardsSummary = getStandardsSummary(findings);
    } catch {
  ...
  saveToHistory(rootPath, scoreResult, suppressions = null) {
    const historyDir = path.join(rootPath, '.praxis');
