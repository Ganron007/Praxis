...
export class ReconAgent extends BaseAgent {
  ...
  async analyze(context) {
    ...
    const MODEL_EXT_SET = new Set(['.pkl', '.pickle', '.pt', '.pth', '.ckpt', '.safetensors', '.gguf', '.onnx', '.h5', '.hdf5', '.pb']);
    const MODEL_BASENAME_RE = /^(?:pytorch_model\.bin|model\.bin|consolidated\..+\.bin)$/;
    ...
    for (const file of files) {
      ...
      // ML model artifacts
      if (MODEL_EXT_SET.has(ext) || MODEL_BASENAME_RE.test(basename)) {
        recon.hasModelFiles = true;
        if (recon.modelFiles.length < 200) recon.modelFiles.push(relPath);
      }
