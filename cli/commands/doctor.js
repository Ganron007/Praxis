...
export async function doctorCommand() {
  console.log();
  console.log(chalk.cyan.bold('  Praxis Doctor'));
  ...
  // 4. API keys
  const apiKeys = [
    { name: 'Anthropic API key', env: 'ANTHROPIC_API_KEY', required: false }, // praxis-ignore — env var names in diagnostic check, no key values
    { name: 'OpenAI API key', env: 'OPENAI_API_KEY', required: false }, // praxis-ignore — env var name in diagnostic check, no key value
  ...
  const ignorePath = path.join(cwd, '.praxisignore');
  if (fs.existsSync(ignorePath)) {
    try {
      const patterns = fs.readFileSync(ignorePath, 'utf-8')
        .split('\n').filter(l => l.trim() && !l.startsWith('#')).length;
      pass(`.praxisignore found (${patterns} patterns)`);
    } catch {
      pass('.praxisignore found');
    }
  } else {
    info('.praxisignore not found (run: praxis init)');
  }

  // 6. Cache directory
  const cacheDir = path.join(cwd, '.praxis');
