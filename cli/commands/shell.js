 ...
 * =================================
 *
 * `praxis shell` drops you into a persistent interactive session.
 *
 * Slash commands:
...
async function praxisBannerAnimated() {
...
export async function shellCommand(targetPath = '.', options = {}) {
  ...
  let running = true;
  while (running) {
    const line = (await ask(chalk.cyan('praxis › '))).trim();
    if (!line) continue;
...
function buildSystemPrompt(state) {
  ...
  return [
    'You are the Praxis security agent embedded in a developer\'s CLI.',
