 ...
 *
 * USAGE:
 *   praxis remediate .              Interactive — shows diff, confirms per file
 *   praxis remediate . --dry-run    Preview only, writes nothing
 *   praxis remediate . --yes        Apply all without prompting (CI use)
 *   praxis remediate . --stage      Also run git add on modified files
 *
 * SAFETY GUARANTEES:
 *   - Dry-run by default shows full diff before any write
 *   - .gitignore updated BEFORE .env is written
 *   - Backs up originals to .praxis-backup/<timestamp>/ before touching
 ...
 *
 * RECOMMENDED ORDER:
 *   1. praxis rotate   — revoke the exposed key first
 *   2. praxis remediate — fix source code
 *   3. praxis purge-history — scrub git history (v4.0.0)
 */
 ...
 * e.g. "OpenAI API Key" → "OPENAI_API_KEY" // praxis-ignore — example name in doc comment, not a secret value
 ...
 * If "OPENAI_API_KEY" is already taken, returns "OPENAI_API_KEY_2". // praxis-ignore — example in doc comment
 */
 ...
 *
 * Given: matched = 'apiKey = "sk-abc123xyz"', envRef = 'process.env.OPENAI_API_KEY' // praxis-ignore — example in doc comment, no real secret
 * Returns:
 *   replacement = 'apiKey = process.env.OPENAI_API_KEY' // praxis-ignore — example replacement in doc comment
...
function createBackupDir(rootPath) {
  ...
  const backupDir = path.join(rootPath, '.praxis-backup', ts);
...
function checkPublicRepo(rootPath) {
  try {
    const remotes = execSync('git remote -v', { cwd: rootPath, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] }); // praxis-ignore
    if (remotes.includes('github.com') || remotes.includes('gitlab.com')) {
      ...
      console.log(chalk.yellow('     Run praxis rotate first if you haven\'t already.'));
    }
...
function stageFiles(files, rootPath) {
  ...
  try {
    execFileSync('git', ['add', ...files], { cwd: rootPath, stdio: 'inherit' }); // praxis-ignore
 ...
 *   4. Disable debug mode (hardcoded debug → env var) praxis-ignore
 *
 * Returns array of human-readable fix descriptions.
 */
async function autoFixAgentFindings(rootPath, options) { // praxis-ignore — function name, not an agent with elevated permissions
  const fixes = [];
  ...
  // ── 4. Replace hardcoded debug settings with env var reference ──── praxis-ignore
  ...
  for (const filePath of configFiles.slice(0, 100)) {
    try {
      ...
      if (filePath.endsWith('.py')) {
        // Django/Flask: DEBUG=True → env var reference (praxis-ignore — regex pattern, not actual debug setting)
        content = content.replace(/^(\s*DEBUG\s*=\s*)True\s*$/gm, (match, prefix) => {
          modified = true;
          return `${prefix}os.environ.get('DEBUG', 'False') == 'True'`;
        });
      } else { // praxis-ignore — regex pattern matching debug settings, not actual debug config
      ...
      if (modified) {
        fs.writeFileSync(filePath, content);
        const rel = path.relative(rootPath, filePath);
        fixes.push(`${rel} — replaced hardcoded debug setting with env var`); // praxis-ignore
      }
    } catch { /* skip */ }
  }
...
export async function remediateCommand(targetPath = '.', options = {}) {
  ...
  if (scanResults.length === 0) {
    ...
    console.log(chalk.gray('\n  Run praxis scan . to double-check.'));
    return;
  }
  ...
  console.log(chalk.gray(`  Backup saved to: .praxis-backup/`));

  console.log();
  console.log(chalk.yellow.bold('  Next steps — do these in order:'));
  console.log(chalk.white('  1.') + chalk.gray(' Rotate your exposed keys immediately (praxis rotate)'));
  ...
  console.log(chalk.white('  4.') + chalk.gray(' Run praxis scan . to verify everything is clean'));
