...
export async function auditCommand(targetPath = '.', options = {}) {
  ...
  // Count suppressions (praxis-ignore comments)
...
function outputJSON(scoreResult, findings, depVulns, recon, agentResults, remediationPlan, suppressions, history) {
  ...
  if (scoreResult.agenticSummary) output.agenticSummary = scoreResult.agenticSummary;
...
async function findFiles(rootPath) {
  ...
  // Load .praxisignore — walk up to the project root so subdirectory scans
  // still honor the repo-level ignore file.
  const ignorePath = findUpwards(rootPath, '.praxisignore');
  if (ignorePath) {
    try {
  ...
  for (const file of files) {
    try {
      ...
      for (const line of lines) {
        if (/praxis-ignore/i.test(line)) {
          total++;
          // Try to extract rule name from comment: praxis-ignore RULE_NAME
          const match = line.match(/praxis-ignore\s+(\w+)/i);
...
function outputMarkdown(scoreResult, findings, depVulns, remediationPlan, rootPath) {
  const lines = [];
  lines.push('# Praxis Security Report');
  lines.push('');
