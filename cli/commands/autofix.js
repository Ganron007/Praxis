 ...
 * ========================================
 *
 * Reads findings from a praxis report (or the last audit run), applies the
 ...
 *
 * USAGE:
 *   npx praxis autofix                        Auto-fix from last report
 *   npx praxis autofix --report report.json   Auto-fix from specific report
 *   npx praxis autofix --dry-run              Preview fixes without applying
 *   npx praxis autofix --severity high        Only fix critical+high findings
 *
 * SAFETY:
...
export async function autofixCommand(options = {}) {
  ...
  console.log();
  output.header('Praxis — Agentic Autofix');
  console.log();

  if (!reportPath || !fs.existsSync(reportPath)) {
    output.error('No report found. Run `npx praxis audit . --deep --json` first, or pass --report <path>.');
  ...
  if (fixable.length === 0) {
    ...
    console.log(chalk.gray('  Tip: Run `npx praxis audit . --deep` to generate AI-powered fix suggestions.'));
    return;
  }
  ...
  const branchName = `praxis/autofix-${timestamp}`;
  ...
  try {
    ...
    const commitMsg = [
      `fix(security): apply praxis autofix annotations`,
      '',
      `Addresses ${applied.length} finding(s) from praxis audit:`,
      ...applied.map(f => `- ${f.severity.toUpperCase()}: ${f.title} (${f.file}${f.line ? `:${f.line}` : ''})`),
      '',
      'Fix suggestions generated by praxis Tier 3 (Opus) deep analysis.',
  ...
  const prTitle = `fix(security): praxis autofix — ${applied.length} finding(s)`;
...
export function applyInlineAnnotation(filePath, lineNum, fix) {
  ...
  if (isPy) {
    annotation = [
      `${indent}# praxis-fix [REVIEW REQUIRED]`,
...
Praxis detected **${applied.length}** security finding(s) and has annotated the affected files with fix instructions. Each annotation is marked \`// praxis-fix [REVIEW REQUIRED]\` — **please review and apply the suggested changes before merging.**
...
> Generated by [praxis]() — AI-powered security scanner.
> Run \`npx praxis audit . --deep\` to regenerate findings.
