 ...
 *
 * USAGE:
 *   praxis rotate .                         Scan local files and rotate found secrets
 *   praxis rotate . --provider github        Only rotate GitHub tokens
 *   praxis rotate --plan rotation-plan.json  Execute a plan from .com/rotate
 *
 * RECOMMENDED ORDER:
 *   1. praxis rotate      ← revoke the key so it can't be used
...
function openBrowser(url) {
  try {
    ...
    if (platform === 'win32') execSync(`start "" "${url}"`, { stdio: 'ignore' }); // praxis-ignore — url is hardcoded provider dashboard URL
    else if (platform === 'darwin') execSync(`open "${url}"`, { stdio: 'ignore' }); // praxis-ignore
    else execSync(`xdg-open "${url}"`, { stdio: 'ignore' }); // praxis-ignore
    return true;
  } catch {
    return false;
  }
...
export async function rotatePlanCommand(planFile) {
  ...
  try {
    ...
    output.error('Failed to parse rotation plan JSON. Make sure it was downloaded from .com/rotate');
    process.exit(1);
  }
  ...
  console.log(chalk.gray('  Run praxis scan . to confirm no hardcoded credentials remain.\n'));
}
...
export async function rotateCommand(targetPath = '.', options = {}) {
  ...
  console.log(chalk.white('  1.') + chalk.gray(' Run praxis remediate . to fix your source code'));
  console.log(chalk.white('  2.') + chalk.gray(' Commit the cleaned files'));
  console.log(chalk.white('  3.') + chalk.gray(' Run praxis scan . to confirm nothing was missed'));
  console.log();
}
