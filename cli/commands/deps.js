...
export async function depsCommand(targetPath = '.', options = {}) {
  ...
  if (options.fix) {
    ...
    try {
      execSync(pm.fixCommand, { cwd: absolutePath, stdio: 'inherit' }); // praxis-ignore — command is a hardcoded package manager command, not user input
    } catch {
...
function runAudit(pm, cwd) {
  let stdout;
  try {
    stdout = execSync(pm.auditCommand, { cwd, stdio: ['pipe', 'pipe', 'pipe'] }).toString(); // praxis-ignore — command is a hardcoded package manager audit command, not user input
  } catch (err) {
...
async function fetchEPSS(cves) {
  ...
  for (const batch of batches) {
    const url = `https://api.first.org/data/v1/epss?cve=${batch.join(',')}`; // praxis-ignore — hardcoded FIRST.org API endpoint, CVE IDs are from audit results not user input
    const response = await fetch(url, { // praxis-ignore — EPSS API fetch with fixed base URL
