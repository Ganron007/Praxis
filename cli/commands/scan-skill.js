 ...
 *
 * USAGE:
 *   praxis scan-skill <url>          Analyze a skill from URL
 *   praxis scan-skill <path>         Analyze a local skill file
 *   praxis scan-skill . --all        Scan all skills in openclaw.json
 */
...
// Praxis tools are added lazily in checkHermesFrontmatter() to avoid
...
export async function scanSkillCommand(target, options = {}) {
  if (!target) {
    output.error('Usage: praxis scan-skill <url|path>');
    output.info('  Analyze an AI agent skill for security issues before installing it.');
    process.exit(1);
  }

  console.log();
  output.header('Praxis — Skill Security Analysis');
  console.log();
...
async function ensureHermesToolsLoaded() {
  ...
  try {
    ...
    for (const t of HERMES_TOOLS) KNOWN_TOOL_REGISTRIES[t.name] = 'praxis';
