 ...
 * ==========
 *
 * Exposes praxis as a Model Context Protocol (MCP) server.
 ...
 *   get_findings    - Read findings from a saved praxis report file
 *   suppress_finding - Add a praxis-ignore comment to suppress a finding
 *
 * PROTOCOL:
...
const TOOLS = [
  ...
  {
    name: 'get_checklist',
    description: 'Return the praxis launch-day security checklist as structured data. Use this to guide users through pre-launch security checks.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        reportPath: {
          type: 'string',
          description: 'Path to the praxis JSON report file (e.g. praxis-report.json).',
        },
        severity: {
          type: 'string',
  ...
  },
  {
    ...
    description: 'Add a praxis-ignore comment to a specific line in a file to suppress a false-positive security finding. The comment tells praxis\'s scanner to skip that line in future scans. Always explain why the suppression is safe.',
    inputSchema: {
      type: 'object',
      properties: {
        file: {
...
function suppressFinding({ file, line, reason }) {
  ...
  // Already suppressed?
  if (/praxis-ignore/i.test(targetLine)) {
  ...
  switch (method) {
    case 'initialize':
      return respond({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'praxis', version: '3.0.0' },
      });
