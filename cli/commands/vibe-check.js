...
export async function vibeCheckCommand(targetPath = '.', options = {}) {
  ...
  // ── Shareable one-liner ──────────────────────────────────────────────────
  const shareLine = `${grade.emoji} ${projectName}: ${score}/100 (${scoreResult.grade.letter}) — ${grade.vibe} vibes | ${allFindings.length} findings | Scanned with Praxis`;
  ...
  if (options.badge) {
    ...
    const badgeMd = `[![Praxis Score](${badgeUrl})]()`;
