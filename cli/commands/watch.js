...
export async function watchCommand(targetPath = '.', options = {}) {
  ...
  console.log();
  output.header('Praxis — Watch Mode');
  console.log();
  ...
  try {
    const watcher = fs.watch(absolutePath, { recursive: true }, (eventType, filename) => { // praxis-ignore — filename from fs.watch OS event, not user input
      ...
      // Skip non-code files
      const ext = path.extname(filename).toLowerCase(); // praxis-ignore — filename from fs.watch OS event
      if (SKIP_EXTENSIONS.has(ext)) return;
      if (SKIP_FILENAMES.has(path.basename(filename))) return; // praxis-ignore
...
async function watchConfigs(absolutePath) {
  console.log();
  output.header('Praxis — Agent Config Watch');
  console.log();
...
function showWatchStatus(rootPath) {
  ...
  if (!fs.existsSync(dbFile)) {
    console.log('\n  No watch data found. Run: praxis watch . --deep\n');
    return;
  }

  try {
    const data = JSON.parse(fs.readFileSync(dbFile, 'utf-8'));
    console.log(`\n  ${chalk.cyan.bold('Praxis Watch — Status')}`);
...
async function watchStateful(absolutePath, options = {}) {
  ...
  console.log();
  output.header('Praxis — Stateful Watch Mode (Kimi K2.6)');
  console.log();
...
async function watchDeep(absolutePath, options = {}) {
  ...
  const slackWebhook  = options.slack      || process.env.PRAXIS_SLACK_WEBHOOK || null;
  const prComments    = options.prComment  || false;
  const scoringEngine = new ScoringEngine();

  console.log();
  output.header('Praxis — Deep Watch Mode');
  console.log();
 ...
 * Webhook URL can be set via --slack or PRAXIS_SLACK_WEBHOOK env var.
 */
async function postSlackAlert(webhookUrl, findings, scoreResult, rootPath) {
  ...
  const payload = {
    attachments: [{
      color,
      fallback: `Praxis: ${findings.length} security finding(s) in ${repoName}`,
      title:    `${emoji} Praxis — Security Alert`,
...
async function postPRComments(findings, rootPath) {
  ...
  for (const f of criticalOrHigh) {
    ...
    const body = [
      ...
      '',
      `_[${f.rule}] — detected by praxis watch_`,
