 ...
 *
 * USAGE:
 *   praxis diff              Scan uncommitted changes (staged + unstaged)
 *   praxis diff --staged     Scan only staged changes
 *   praxis diff HEAD~3       Scan changes in last 3 commits
 *   praxis diff main         Scan changes since branching from main
 */
...
export async function diffCommand(ref, options) {
  ...
  console.log(chalk.cyan.bold('  Praxis — Diff Scan'));
  console.log(chalk.cyan('═'.repeat(60)));
  console.log();
