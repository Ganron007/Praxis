...
async function handleHermesInit(targetDir, options) {
  ...
  const TRUSTED_HOSTS = ['.com', 'www..com', 'localhost', '127.0.0.1'];
  if (!TRUSTED_HOSTS.includes(parsed.hostname)) {
    console.error(chalk.red('\nError: Setup URL must be from .com (got: ' + parsed.hostname + ')'));
    console.error(chalk.gray('  Only URLs generated by the Praxis webapp are trusted.\n'));
    process.exit(1);
  }
