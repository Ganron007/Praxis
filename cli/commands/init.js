 ...
 * Copies pre-configured security files from praxis.
 *
 * USAGE:
 *   praxis init              Copy all security configs
 *   praxis init --gitignore  Only copy .gitignore
 *   praxis init --headers    Only copy security headers
 *   praxis init -f           Force overwrite existing files
 *
 * FILES COPIED:
...
async function handleGitignore(targetDir, force, results) {
  ...
  if (!fs.existsSync(sourcePath)) {
    results.errors.push({
      file: '.gitignore',
      error: 'Source file not found in praxis package'
    });
    return;
  }
  ...
  if (fs.existsSync(targetPath)) {
    if (force) {
      ...
      // Merge: append praxis patterns to existing
      const existingContent = fs.readFileSync(targetPath, 'utf-8');

      // Check if already has praxis content
      if (existingContent.includes('# PRAXIS')) {
        results.skipped.push('.gitignore (already contains praxis patterns)');
        return;
      }

      // Append praxis section
...
async function handleAgentFiles(targetDir, force, results) {
  ...
  for (const { file, label } of appendTargets) {
    ...
    if (fs.existsSync(targetPath)) {
      ...
      if (existing.includes(AGENT_MARKER)) {
        results.skipped.push(`${label} (already contains praxis rules)`);
        continue;
      }
  ...
  const cursorRulesFile = path.join(cursorRulesDir, 'praxis.mdc');

  if (fs.existsSync(cursorRulesFile) && !force) {
    results.skipped.push('.cursor/rules/praxis.mdc (already exists, use -f to overwrite)');
  } else {
    if (!fs.existsSync(cursorRulesDir)) {
      fs.mkdirSync(cursorRulesDir, { recursive: true });
    }
    fs.writeFileSync(cursorRulesFile, CURSOR_RULE_CONTENT.trim() + '\n');
    results.copied.push('.cursor/rules/praxis.mdc');
  }
}
...
async function handleHermesInit(targetDir, options) {
  ...
  // Validate the URL is well-formed and from a trusted origin.
  // Allow HTTPS (any host — the security is HTTPS + explicit user opt-in via
  // `praxis init --from <url>`) and localhost for local development.
  let parsed;
  try {
  ...
  if (!isLocal && !isHttps) {
    console.error(chalk.red('\nError: Setup URL must use HTTPS (got: ' + parsed.protocol + '//)'));
    console.error(chalk.gray('  Or use http://localhost for local development.\n'));
    process.exit(1);
  }
...
const HARDENED_OPENCLAW = `{
  "// SECURITY": "Generated by praxis init --openclaw — hardened defaults",
...
function printSummary(results) {
  ...
  if (results.merged.length > 0) {
    ...
    for (const file of results.merged) {
      console.log(chalk.blue(`  \u2194 ${file} (appended praxis patterns)`));
    }
    console.log();
  }
