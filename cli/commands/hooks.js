 ...
 * =============
 *
 * Installs praxis as PreToolUse / PostToolUse hooks in Claude Code
 * (~/.claude/settings.json). Once installed, praxis runs automatically
 ...
 *   ~/.praxis/hooks/ (a stable, user-owned directory) and register those
...
const STABLE_HOOK_DIR = path.join(os.homedir(), '.praxis', 'hooks');
...
function install() {
  ...
  for (const script of scripts) {
    ...
    if (!fs.existsSync(src)) {
      console.error(chalk.red(`Hook script not found: ${src}. Try reinstalling praxis.`));
      process.exit(1);
    }
  }
  ...
  const preEntry = buildEntry(
    ...
    PRE_COMMAND,
    'praxis pre-tool-use: block secrets in writes, dangerous bash patterns'
  );
  ...
  const postEntry = buildEntry(
    ...
    POST_COMMAND,
    'praxis post-tool-use: advisory scan after file writes'
  );
  ...
  if (!changed) {
    console.log(chalk.green('✔ praxis hooks are already installed.'));
  ...
  console.log(chalk.green.bold('\n✔ praxis hooks installed successfully.\n'));
...
function remove() {
  ...
  if (removed === 0) {
    console.log(chalk.yellow('No praxis hooks found in settings.json.'));
    return;
  }
...
function printStatus(settings) {
  ...
  console.log(chalk.bold('\npraxis Claude Code hooks status:\n'));
  console.log(
