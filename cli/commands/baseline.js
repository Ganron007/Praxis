 ...
 *
 * USAGE:
 *   praxis baseline .          Create a new baseline from current scan
 *   praxis baseline . --diff   Show what changed since baseline
 *   praxis baseline . --clear  Remove the baseline
 */
  ...
  for (const file of filtered) {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (/praxis-ignore/i.test(lines[i])) continue;
...
async function fullScan(rootPath) {
  ...
  const { findings: agentFindings } = await orchestrator.runAll(rootPath, { quiet: true }); // praxis-ignore — orchestrator result, not LLM output triggering actions
...
export async function baselineCommand(targetPath = '.', options = {}) {
  ...
  if (options.diff) {
    const baseline = loadBaseline(rootPath);
    if (!baseline) {
      console.log(chalk.yellow('  No baseline found. Run `praxis baseline .` first.'));
      return;
    }
  ...
  console.log();
  console.log(chalk.gray('  Run `praxis audit . --baseline` to only see new findings.'));
}
